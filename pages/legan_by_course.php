...
$code = $_GET['schid'];
$coureCode = $_GET['code'];
$Ccode="SELECT * FROM `courses` WHERE `CourseCode`='$coureCode'";
$Ccode2 = mysqli_query($bis,$Ccode) or die(mysqli_error($bis));
$Ccode1 = mysqli_fetch_assoc($Ccode2);
...
$query_leganList = "SELECT p31_students_legan.lagna_no , count(*), legan_list.lagna_name  FROM p31_students_legan, legan_list WHERE p31_students_legan.lagna_no = legan_list.lagna_no AND p31_students_legan.schid = $code  group by p31_students_legan.lagna_no";
...
$query_legan_doctors = "SELECT p31_students_legan.group_id , count(*), doctorscourse.DoctorCode, doctors_account.NameAr FROM p31_students_legan, doctorscourse, doctors_account WHERE p31_students_legan.schid = $code AND p31_students_legan.lagna_no = $legan_id AND doctorscourse.id = p31_students_legan.group_id AND doctors_account.DoctorCode=doctorscourse.DoctorCode  GROUP BY doctorscourse.DoctorCode";
...
$totalRows_legan_doctors = mysqli_num_rows($legan_doctors);
        ...
        <tr align="center" valign="middle" <?php if($totalRows_legan_doctors >1 ) { ?>  bgcolor="#99FF99" <?php } ?>>
          ...
          <th width="12%" height="30" align="center" valign="middle" scope="row"><a href="assign_observers.php?lagna=<?php echo $row_leganList['lagna_no'];?>&amp;code=<?php echo $Ccode1['CourseCode'];?>&amp;date=<?php echo $row_legandata['exam_date'];?>"><span class="style23"> تسكسين المراقبين </span></a></th>
